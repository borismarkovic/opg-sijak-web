...
> **Usage:** This file defines the visual language for the OPG Šijak website.
> Design tokens (colors, spacing, shadows, radii) should be mapped to Tailwind v4 `@theme` variables.
> Component CSS specs below are **visual reference only** — actual implementation uses shadcn/ui + Tailwind.

---

**Project:** OPG Šijak
**Generated:** 2026-04-14 20:57:00
...
| Token | Value | Usage |
|-------|-------|-------|
| `--radius-sm` | `8px` / `0.5rem` | Small chips, tags |
| `--radius-md` | `12px` / `0.75rem` | Buttons, inputs |
| `--radius-lg` | `16px` / `1rem` | Cards, containers |
| `--radius-xl` | `24px` / `1.5rem` | Modals, heroes, featured elements |

---

## Component Specs

> **Note:** These CSS specs are *visual reference only*. The actual implementation uses **shadcn/ui** components styled via Tailwind classes. Do not copy-paste these CSS blocks — use them to understand the intended look and feel, then apply via Tailwind utilities.

### Buttons

```css
/* Primary Button — visual reference */
.btn-primary {
  background: #CA8A04;
  color: white;
  padding: 12px 24px;
  border-radius: 12px;        /* organic feel */
  font-weight: 600;
...
/* Secondary Button — visual reference */
.btn-secondary {
...
.card {
  background: #F0FDF4;
  border-radius: 16px;         /* organic feel */
  padding: 24px;
...
> **Note:** No modals are currently planned in the site. Kept for future reference.
